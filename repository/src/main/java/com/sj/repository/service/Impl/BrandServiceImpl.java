...
public class BrandServiceImpl implements BrandService {
	...
	public void updateShowOnIndex(Long id) {
		...
		if (brand.getShowOnIndex() == ActivateEnum.ACTIVATE) {
			...
			brand.setShowOnIndex(ActivateEnum.ACTIVATE);
			brand.setShowTime(Calendar.getInstance());
		}
	...
	public Page<Brand> searchBrand(BrandSearchOption option, Pageable pageable) {
		return repository.findByNameLike("%" + option.getTitle() + "%",
				pageable);
	}
	...
	public List<Field> filterNullValue(BrandSearchOption option, Field[] fields) {
