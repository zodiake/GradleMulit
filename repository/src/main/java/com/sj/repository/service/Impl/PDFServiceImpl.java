...
public class PDFServiceImpl implements PDFService {
	...
	private PdfPCell getProductCell(String content, int colspan,
			Font fontChinese) {
		PdfPCell c = new PdfPCell(new Paragraph(content, fontChinese));
		c.setFixedHeight(38);
		c.setPaddingLeft(10);
	...
	private PdfPTable getProductTable(Set<BuyProduct> buyProducts,
			Font fontChinese, float totalPrice) throws DocumentException {
		...
		for (BuyProduct buyProduct : buyProducts) {
			...
			table.addCell(getCell(buyProduct.getNumber().toString(), 1,
					fontChinese));
			table.addCell(getCell(
					String.valueOf(buyProduct.getProduct().getPrice()), 1,
					fontChinese));
	...
	@Override
	public byte[] getBuyRecordPdf(BuyRecord buyRecord, OutputStream out,
			String type) throws DocumentException, ParseException, IOException {
		...
		case "one":
			...
			PdfWriter writeOne = PdfWriter.getInstance(documentOne,
		...
		case "two":
			...
			Document documentTwo = new Document(rectangle.rotate(), 0f, 0f, 10,
					10);
			PdfWriter writeTwo = PdfWriter.getInstance(documentTwo,
					byteArrayOutputStream);
			typeTwo(buyRecord, baseFontChinese, fontChinese, documentTwo);
			byteArrayOutputStream.writeTo(out);
			byte[] bytesTwo = byteArrayOutputStream.toByteArray();
			addWatermarkTwo(bytesTwo, byteArrayOutputStream, writeTwo);
			break;
		}
	...
	private void typeTwo(BuyRecord buyRecord, BaseFont baseFontChinese,
		...
		infoTable.addCell(new PdfPCell(new Paragraph("订单编号：", fontChinese)))
				.setBorderColor(new BaseColor(255, 255, 255));
		infoTable.addCell(
				new PdfPCell(new Paragraph(String.valueOf(buyRecord.getNoId()),
						fontChinese))).setBorderColor(
				new BaseColor(255, 255, 255));
		infoTable.addCell(new PdfPCell(new Paragraph("订单日期：", fontChinese)))
				.setBorderColor(new BaseColor(255, 255, 255));
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日");
		infoTable.addCell(
				new PdfPCell(new Paragraph(sdf.format(buyRecord.getCreateTime()
						.getTime()), fontChinese))).setBorderColor(
				new BaseColor(255, 255, 255));
		infoTable.addCell(new PdfPCell(new Paragraph("订单金额：", fontChinese)))
				.setBorderColor(new BaseColor(255, 255, 255));
		infoTable.addCell(
				new PdfPCell(new Paragraph(
						String.valueOf(buyRecord.getPrice()), fontChinese)))
				.setBorderColor(new BaseColor(255, 255, 255));
		infoTable.setWidthPercentage(30);
		infoTable.setHorizontalAlignment(Element.ALIGN_RIGHT);
		document.add(infoTable);

		String[] headers = new String[] { "序号", "商品名称", "货号", "品牌", "规格",
				"价格(元)", "数量" };
		...
		for (BuyProduct buyProduct : products) {
			...
			productsTable.addCell(getBorderNoColor(String.valueOf(no),
					fontChinese));
			...
			productsTable.addCell(getBorderNoColor(p.getBrand().getName(),
					fontChinese));
			productsTable.addCell(getBorderNoColor(p.getSpecifications(),
					fontChinese));
			productsTable.addCell(getBorderNoColor(
					String.valueOf(p.getPrice()), fontChinese));
			productsTable.addCell(getBorderNoColor(
					String.valueOf(buyProduct.getNumber()), fontChinese));
			no++;
		}
		Paragraph countP = new Paragraph("小计：" + buyRecord.getPrice(),
				fontChinese);
		...
		document.add(new Paragraph(
				"     非常感谢您在优捷实验(Ulabexpress.com)采购，我们期待您的再次光临。", fontChinese));
		document.add(new Paragraph(
				"     如果您想了解我们的商品验收、退换货政策/流程，请与我们的客服人员进行联系。", fontChinese));
		document.add(new Paragraph(""));
		document.add(new Paragraph("     备注：", fontChinese));
		document.add(new Paragraph(
				"     1.供货期的前提为国外有现货，仓库现货以即时为准，对于停产和禁运商品我们将及时通知。", fontChinese));
		...
		for (BuyProduct buyProduct : buyRecord.getProducts()) {
			...
			Product product = productRepository.findOne(buyProduct.getProduct()
					.getId());
	...
	private void typeOne(BuyRecord buyRecord, BaseFont baseFontChinese,
			...
			ParseException, IOException, MalformedURLException {
		document.open();
		Paragraph p = new Paragraph("         编号：" + buyRecord.getNoId(),
				fontChinese);
	...
	private PdfPTable getTable(BuyRecord buyRecord, Font fontChinese)
		...
		table.addCell(getCell(buyRecord.getUser().getIndustryInfo().getName(),
				1, fontChinese));
		table.addCell(getCell("申请部门", 1, fontChinese));
		table.addCell(getCell(buyRecord.getUser().getDepartment(), 1,
				fontChinese));
		...
		table.addCell(getCell(sdf.format(buyRecord.getCreateTime().getTime()),
				1, fontChinese));
		...
		PdfPCell c = new PdfPCell(getProductTable(buyRecord.getProducts(),
				fontChinese, buyRecord.getPrice()));
		c.setColspan(4);
		c.setPaddingLeft(5);
		c.setPaddingRight(5);
		table.addCell(c);
	...
	private byte[] addWatermark(byte[] bytes, OutputStream out, PdfWriter write)
	...
	private byte[] addWatermarkTwo(byte[] bytes, OutputStream out, PdfWriter write)
			throws DocumentException, IOException {
		PdfReader reader = new PdfReader(bytes);
		PdfStamper stamper = new PdfStamper(reader, out);
		PdfContentByte under = null;
	...
	private PdfPTable getProductsTable(Product product, Font fontChinese)
		...
		infoTable.addCell(getProductCell(String.valueOf(product.getPrice()), 1,
				fontChinese));
		...
		infoTable.addCell(getProductCell(product.getBrand().getName(), 1,
				fontChinese));
		...
		infoTable
				.addCell(getProductCell(
						sdf.format(product.getCreatedTime().getTime()), 1,
						fontChinese));
